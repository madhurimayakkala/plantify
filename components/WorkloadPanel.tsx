...
export default function WorkloadPanel({ workload, onUpdate }: Props) {
  function updateTask(taskId: string, delta: number) {
    const updated: ActiveWorkload = {
      ...
      tasks: workload.tasks.map((t) => {
        ...
        const next = Math.max(0, Math.min(t.total, t.completed + delta));
    ...
    return (
      ...
      >
        <p className="text-sm" style={{ color: "#9b8878" }}>
          No tasks yet. Add a workload to start tracking progress.
        </p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="divide-y" style={{ borderColor: "#f0e8e0" }}>
        {workload.tasks.map((task: Task, i: number) => {
          const pct =
            task.total > 0
            ...
            >
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  >
                    {task.completed} / {task.total} completed · {pct}%
                  </p>
                </div>
